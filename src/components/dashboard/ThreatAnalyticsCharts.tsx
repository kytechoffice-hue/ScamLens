  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 p-5 sm:p-6 rounded-3xl bg-[#0c131f]/90 border border-slate-800 shadow-xl flex flex-col justify-between">
          ...
          {activeTab === "7D" ? (
            <div className="w-full">
              <div className="h-64 sm:h-72 w-full relative flex items-end justify-between gap-2 sm:gap-4 pt-6 pb-2 px-2">
                ...
                <div className="absolute inset-x-0 top-6 border-b border-slate-800/60 flex items-center justify-between text-[9px] font-mono text-slate-600">
                  <span>3,000</span>
                </div>
                <div className="absolute inset-x-0 top-1/2 border-b border-slate-800/60 flex items-center justify-between text-[9px] font-mono text-slate-600">
                  <span>1,500</span>
                </div>
                <div className="absolute inset-x-0 bottom-6 border-b border-slate-800/60 flex items-center justify-between text-[9px] font-mono text-slate-600">
                  <span>0</span>
                </div>
                  ...
                  return (
                    <div key={item.day} className="flex-1 flex flex-col items-center h-full justify-end group relative z-10">
                      ...
                      <div className="absolute -top-10 opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-lg bg-slate-900 border border-slate-700 text-[10px] font-mono text-white shadow-xl pointer-events-none whitespace-nowrap z-20">
                        {item.date}: <strong className="text-cyan-400">{item.threatsDetected}</strong> detections
                      </div>

                      {/* Bar Pillar */}
                      <div className="w-full max-w-[38px] rounded-t-xl overflow-hidden bg-slate-900 flex flex-col justify-end border-t border-x border-cyan-500/30 group-hover:border-cyan-400 transition-all">
                        <div
                          ...
                          className={`w-full transition-all duration-500 rounded-t-lg ${
                            isPeak
                              ...
                              : "bg-gradient-to-t from-slate-800 to-cyan-600/70 group-hover:from-slate-700 group-hover:to-cyan-400"
                          }`}
                        />
                      </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-800 text-xs font-mono text-slate-400">
                ...
                <span className="text-rose-400 font-semibold">↑ 18.4% Week-over-Week</span>
              </div>
            </div>
          ) : (
            <div className="w-full">
                ...
                {last30DaysTrend.map((item, idx) => {
                  ...
                  return (
                    <div key={item.period} className="flex-1 flex flex-col items-center h-full justify-end group relative z-10">
                      ...
                      <div className="w-full max-w-[60px] rounded-t-xl bg-slate-900 overflow-hidden flex flex-col justify-end border-t border-x border-amber-500/30">
                        <div
                          style={{ height: `${heightPercent}%` }}
                          className="w-full bg-gradient-to-t from-amber-700/60 to-amber-400 transition-all duration-500 rounded-t-lg group-hover:to-amber-300"
                        />
                      </div>

                      <span className="text-xs font-mono text-slate-300 mt-2 font-bold">
                        {item.period}
                      </span>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-800 text-xs font-mono text-slate-400">
                <span>Total 30-Day Volume: <strong>12,638 Reports</strong></span>
                <span className="text-amber-400 font-semibold">₹2,480 Lakhs Prevented</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-4 p-5 sm:p-6 rounded-3xl bg-[#0c131f]/90 border border-slate-800 shadow-xl flex flex-col justify-between">
          <div>
          ...
          <div className="pt-4 border-t border-slate-800 text-[11px] font-mono text-slate-400">
            Total Analyzed Vectors: <strong className="text-white">2,847 Active</strong>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-5 sm:p-6 rounded-3xl bg-[#0c131f]/90 border border-slate-800 shadow-xl">
          <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-800">
            <h3 className="text-sm sm:text-base font-bold text-white flex items-center gap-2">
          ...
          <div className="w-full h-4 rounded-xl overflow-hidden flex bg-slate-900 border border-slate-800 mb-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs font-mono">
            {categoryShare.map((cat) => (
              <div key={cat.name} className="flex items-center gap-2 p-2 rounded-xl bg-slate-950/60 border border-slate-850">
                <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: cat.color }} />
                <span className="text-slate-300 truncate">{cat.name}</span>
                <span className="ml-auto font-bold text-white">{cat.percentage}%</span>
              </div>
            ))}
          </div>
        </div>
